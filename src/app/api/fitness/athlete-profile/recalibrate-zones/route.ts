...
/**
 * GET /api/fitness/athlete-profile/recalibrate-zones
 * Read-only preview: what the seasonally-adjusted zones *would* be.
 * Safe to call on page load — never writes.
 */
 ...
 * Recalculates AND PERSISTS HR zones based on current season and stored max HR
 * ceiling. Because this writes a cardiac-safety value, it must only run on an
 * explicit user action — use GET for read-only previews.
 */
